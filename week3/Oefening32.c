...
int main( void )
{
	int counter = 0;
	// TODO : Print header
  printf("char dec   hex | char dec   hex | char dec   hex | char dec   hex\n");
	// TODO : Print lijn
  printf("-------------------------------------------------------------------\n");
	while( counter <= 127 )
	{
    ...
    for(int i = 0; i < 4 && counter <=127; i++)
    {
      if(counter < 32 || counter == 127)
      {
        printf("     %3d  0x%02x | ", counter, counter);
      } else {
        printf("  %c  %3d  0x%02x | ", counter, counter, counter);
      }
      ++counter;
    }
    printf("\n");
